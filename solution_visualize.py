...
class TrafficViolationDetector:
    ...
    def predict(self, image_path, visualize=False, output_path="output.jpg"):
        try:
            ...
            for i, bike in enumerate(bikes):
                ...
                if visualize:
                    if is_triple and is_no_helmet:
                        bike_color = (0, 0, 220)
                        bike_label = f"VIOLATION: {rider_count} riders + no helmet"
                    elif is_triple:
                        bike_color = (0, 0, 220)
                        bike_label = f"VIOLATION: {rider_count} riders"
                    elif is_no_helmet:
                        bike_color = (0, 80, 255)
                        bike_label = f"VIOLATION: no helmet ({helmet_violations})"
                    else:
                        bike_color = (255, 120, 0)
                        bike_label = f"Riders: {rider_count}"
                    ...
                    for rider_box, status in rider_statuses:
                        ...
                        if status == "no_helmet":
                            ...
                            rider_color = (180, 180, 0)
                            rider_label = "Helmet Unknown"

                        cv2.rectangle(output, (rx1, ry1), (rx2, ry2), rider_color, 2)
                        cv2.putText(
                            output,
                            rider_label,
                            (rx1, max(25, ry1 - 10)),
